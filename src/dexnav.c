...
void PopulateEggMoveBuffer(u16 eggSpecies, u16 *eggmoveBuff)
{
    ...
    u16 i;
    for (i = 0; i < EGG_MOVE_TABLE_SIZE; ++i)
	{
        if (gEggMoves[i] == (eggSpecies + 20000)) //  lol gamefreak
		{
            Memcpy((void *)eggmoveBuff, (void*)&gEggMoves[i + 1], 2 * EGGMOVE_MAX);
            return;
        }
...
{	
    ...
    while (element != 0xFF)
	{
        ...
        if ((counter == 0) || (*(gStringVar4 + 1) == 0xFF))
		{
            ...
            Memcpy((void*)dst, (void*)(&dexnavStarsTiles[index]), TILE_SIZE);
        }
		else if ((element == 0x0))
		{
            Memcpy((void*)dst, (void*)(&dexnavStarsTiles[index]), TILE_SIZE);
            u8 *prevLetter = (u8*)(&dexnavStarsTiles[prevIndex]);
            *(dst + 0) = *(prevLetter + 2);
            *(dst + 4) = *(prevLetter + 6);
            *(dst + 8) = *(prevLetter + 10);
...
void DexNavSightUpdate(u8 sight)
{
    ...
    Memcpy((void*)toWrite, (void*) &(dexnavStarsTiles[((19 * 4) + (7 - (2 * sight))) * 32]), 64);

    // draw the B button tile
    Memcpy((void*)(toWrite + 128), (void*) &(dexnavStarsTiles[((19 * 4) + 2) *32]), 64);
...
void DexNavDrawSight(u8 sight_lvl, u8* objidAddr)
{
    ...
    struct SpriteSheet sight_tiles = {(const u8*)(&emptyTiles), 0x800, 0x5424};
...
void DexNavDrawAbility(u8 ability, u8* objidAddr)
{
    ...
    struct SpriteSheet FontSpriteAbility = {(const u8*)(&emptyTiles), 0x800, 0x1EE7};
...
{
    // create empty object of size 32x64 to draw font on
    struct SpriteSheet FontSpriteMove = {(const u8*)(&emptyTiles), 0x800, 0x4736};
...
void DexNavDrawPotential(u8 potential, u8* objidAddr) 
{
    ...
    struct SpriteSheet StarIconLit = {(const u8*)(&(dexnavStarsTiles[19 * 4 * 32])), 64, 0x61};
    struct SpriteSheet StarIconOff = {(const u8*)(&(dexnavStarsTiles[((19 * 4) + 1) *32])), 64, 0x2613};
...
void DexNavGuiHandler(void)
{
	switch(gMain.state)
	{
        ...
        case 1:
		{
				...
				LoadCompressedPalette((void*) dexnavGuiPal, 0, 32);				
				LZ77UnCompWram((void*) dexnavGuiMap, (void*) DexNav_gbackBuffer);
				LZ77UnCompVram((void*) dexnavGuiTiles, (void*) 0x06000000);
