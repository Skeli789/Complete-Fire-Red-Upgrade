...
static u32 DetermineEggPersonality(struct DayCare *daycare, struct BoxPokemon* mother)
{
	...
	if (!((struct Pokemon*) mother)->hiddenAbility && GetBoxMonData(mother, MON_DATA_SPECIES, NULL) != SPECIES_DITTO && Random() % 100 < 80) //80 % chance to pass down slotted ability
...
{	
	#ifdef UNBOUND
		u16 numEggs = VarGet(DAYCARE_NUM_EGGS_VAR);
		if (numEggs < 100)
			++numEggs;
		VarSet(DAYCARE_NUM_EGGS_VAR, numEggs); 
	#else
