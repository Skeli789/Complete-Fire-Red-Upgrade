...
extern void CB2_DoHallOfFameScreen(void);
extern void Task_Hof_InitTeamSaveData(u8 taskId);
extern void Task_Hof_DisplayMon(u8 taskId);
extern void Task_Hof_PrintMonInfoAfterAnimating(u8 taskId);
extern void Task_Hof_TryDisplayAnotherMon(u8 taskId);
extern void Task_HofPC_CopySaveData(u8 taskId);
extern void Task_HofPC_DrawSpritesPrintText(u8 taskId);
extern void Task_HofPC_PrintMonInfo(u8 taskId);
extern void Task_Hof_InitMonData(u8 taskId);
extern void HallOfFame_PrintMonInfo(struct HallofFameMon *currMon, unusedArg u8 a1, unusedArg u8 a2);
...
void CB2_DoHallOfFameScreen(void) {
    if (!InitHallOfFameScreen()) {
        ...
        gHofMonPtr = Calloc(sizeof(*sHofMonPtr));
	}
	return;
};
...
/*
void FameHallScreen(void);
void FameHallSaveTeamData(u8 taskId);
void FameHallDisplayMon(u8 taskId);
void FameHallPrintMonInfoAfterAnimation(u8 taskId);
void FameHallTryNewMon(u8 taskId);
void FameHallCopySaveData(u8 taskId);
void FameHallDrawAndPrint(u8 taskId);
void FameHallPrintMonInfo(u8 taskId);
void FameHallInitMonData(u8 taskId);
void FameHallPrintInfo(struct HallofFameMon *currMon, u8 a1, u8 a2);


void FameHallScreen(void) {
    if (!InitHallOfFameScreen())
	{
        u8 taskId = CreateTask(FameHallInitMonData, 0);
        gTasks[taskId].data[0] = FALSE;
        //gHallOfFameMonData = Calloc(sizeof(gHallOfFameMonData));
...
void FameHallSaveTeamData(u8 taskId) {
    ...
    *lastSavedTeam = *gHallOfFameMonData;

    DrawDialogueFrameNewMenuHelpers(0, 0);
    AddTextPrinterParameterized2(0, 1, gText_SavingDontTurnOffPower, 0, NULL, 2, 1, 3);
    CopyWindowToVram(0, 3);
    gTasks[taskId].func = HallOfFameTrySaveData;
};
...
void FameHallDisplayMon(u8 taskId) {
    ...
    struct HallofFameMon *currMon = &gHallOfFameMonData->mon[currMonId];

    if (gTasks[taskId].data[2] > 3)
	{
        xPos = HallofFameMonFullTeamPositions[currMonId][0];
        yPos = HallofFameMonFullTeamPositions[currMonId][1];
        field4 = HallofFameMonFullTeamPositions[currMonId][2];
        field6 = HallofFameMonFullTeamPositions[currMonId][3];
    }
	else
	{
        xPos = HallofFameMonHalfTeamPositions[currMonId][0];
        yPos = HallofFameMonHalfTeamPositions[currMonId][1];
        field4 = HallofFameMonHalfTeamPositions[currMonId][2];
    ...
    gSprites[spriteId].callback = SpriteCbAnimateOnScreen;
    gTasks[taskId].data[currMonId + 5] = spriteId;
    DeleteWindow(0, TRUE);
    gTasks[taskId].func = FameHallPrintMonInfoAfterAnimation;
};
...
void FameHallPrintMonInfoAfterAnimation(u8 taskId) {
    ...
    if (monSprite->data[0]) {
        ...
        gTasks[taskId].func = FameHallTryNewMon;
    }
};
...
void FameHallTryNewMon(u8 taskId) {
    ...
    struct HallofFameMon *currMon = &gHallOfFameMonData->mon[currPokeID];
	...
	else
	{
        ...
        if (gTasks[taskId].data[1] <= 4 && currMon[1].species != SPECIES_NONE)  // there is another pokemon to display
        {
            ...
            gTasks[taskId].func = FameHallDisplayMon;
        }
		else
		{
            gTasks[taskId].func = HallOfFamePrintWelcomeTextTask;
        }
    }
};
...
void FameHallCopySaveData(u8 taskId) {
    CreateHallOfFameWindow(0, 0x1E, 0, 0xC, 0x226);
    if (SaveLoadGameData(3) != 1) {
        gTasks[taskId].func = TaskHallOfFamePrintCorruptedData;
    } else {
        u16 i;
        struct HallofFameTeam *savedTeams;
        ...
        gTasks[taskId].func = FameHallDrawAndPrint;
    }
};


void FameHallDrawAndPrint(u8 taskId) {
    struct HallofFameTeam *savedTeams = gHallOfFameMonData;
    struct HallofFameMon *currMon;
    u16 i;
    ...
    for (i = 0; i < PARTY_SIZE; ++i, ++currMon)
	{
        if (currMon->species != 0)
		{
			...
			{
                posX = HallofFameMonFullTeamPositions[i][2];
                posY = HallofFameMonFullTeamPositions[i][3];
            }
			else
			{
                posX = HallofFameMonHalfTeamPositions[i][2];
    ...
    if (gTasks[taskId].data[0] <= 0)
        HallOfFamePutText(gStringVar4, gText_PickCancel, 0, 0, TRUE);
    else
        HallOfFamePutText(gStringVar4, gText_PickNextCancel, 0, 0, TRUE);

    gTasks[taskId].func = FameHallPrintMonInfo;
};
...
void FameHallPrintMonInfo(u8 taskId) {
    ...
    gSprites[currMonID].oam.priority = 0;
    gHallOfFameFadingRelated = (0x10000 << gSprites[currMonID].oam.paletteNum) ^ 0xFFFF0000;
    ...
    gTasks[taskId].func = TaskHallOfFameHandleInput;
};
...
void FameHallInitMonData(u8 taskId) {
    ...
    gTasks[taskId].data[2] = 0;  // valid pokes
    ...
    for (i = 0; i < PARTY_SIZE; ++i)
	{
        ...
        if (GetMonData(&gPlayerParty[i], MON_DATA_SPECIES, NULL))
		{
            gHallOfFameMonData->mon[i].species = GetMonData(&gPlayerParty[i], MON_DATA_SPECIES2, NULL);
            gHallOfFameMonData->mon[i].tid = GetMonData(&gPlayerParty[i], MON_DATA_OT_ID, NULL);
            gHallOfFameMonData->mon[i].personality = GetMonData(&gPlayerParty[i], MON_DATA_PERSONALITY, NULL);
            gHallOfFameMonData->mon[i].lvl = GetMonData(&gPlayerParty[i], MON_DATA_LEVEL, NULL);
            GetMonData(&gPlayerParty[i], MON_DATA_NICKNAME, nick);
            for (j = 0; j < POKEMON_NAME_LENGTH; j++)
			{
                gHallOfFameMonData->mon[i].nick[j] = nick[j];
            }
            gTasks[taskId].data[2]++;
        } 
		else
		{
            gHallOfFameMonData->mon[i].species = 0;
            gHallOfFameMonData->mon[i].tid = 0;
            gHallOfFameMonData->mon[i].personality = 0;
            gHallOfFameMonData->mon[i].lvl = 0;
            gHallOfFameMonData->mon[i].nick[0] = EOS;
        }
    }
    ...
    if (gTasks[taskId].data[0])
        gTasks[taskId].func = HallOfFameSetMonDisplayTask;
    else
        gTasks[taskId].func = FameHallSaveTeamData;
};



void FameHallPrintInfo(struct HallofFameMon *currMon, unusedArg u8 a1, unusedArg u8 a2) {
    u8 text[30];
    u8 *stringPtr;
    s32 dexNumber;
    s32 width;
