	...
	case EFFECT_SPIKES:;
		...
		case MOVE_TOXICSPIKES:
			for (i = 0; i < PARTY_SIZE; ++i)
			{
				...
				{
					u8 type1 = (gBattleTypeFlags & BATTLE_TYPE_CAMOMONS) ? GetCamomonsTypeByMon(&defParty[i], 0) : gBaseStats2[defParty[i].species].type1;
					u8 type2 = (gBattleTypeFlags & BATTLE_TYPE_CAMOMONS) ? GetCamomonsTypeByMon(&defParty[i], 1) : gBaseStats2[defParty[i].species].type2;
