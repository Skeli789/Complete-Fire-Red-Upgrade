...
#define IS_MOLD_BREAKER(ability, move) (ability == ABILITY_MOLDBREAKER || ability == ABILITY_TERAVOLT || ability == ABILITY_TURBOBLAZE || gSpecialMoveFlags[move].gMoldBreakerMoves)
...
#define STAT_CAN_RISE(bank, stat) ((gBattleMons[bank].statStages[stat-1] < STAT_STAGE_MAX && ABILITY(bank) != ABILITY_CONTRARY) || (ABILITY(bank) == ABILITY_CONTRARY && gBattleMons[bank].statStages[stat-1] > 0))
#define STAT_CAN_FALL(bank, stat) ((gBattleMons[bank].statStages[stat-1] > 0 && ABILITY(bank) != ABILITY_CONTRARY) || (ABILITY(bank) == ABILITY_CONTRARY && gBattleMons[bank].statStages[stat-1] < STAT_STAGE_MAX))
